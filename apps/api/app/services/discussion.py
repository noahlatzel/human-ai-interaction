...
from app.models import Discussion, DiscussionReply, DiscussionSubscription, Notification, User
from app.api.schemas.discussion import DiscussionCreate, DiscussionReplyCreate


# Discussion CRUD
async def create_discussion(db: AsyncSession, discussion: DiscussionCreate, author_id: str) -> Discussion:
...
async def get_reply_count(db: AsyncSession, discussion_id: int) -> int:
    ...
    query = select(func.count()).select_from(DiscussionReply).filter(DiscussionReply.discussion_id == discussion_id)
    result = await db.execute(query)
...
async def create_reply(
    ...
    await create_reply_notifications(db, discussion_id=discussion_id, reply_id=db_reply.id, author_id=author_id)
...
# Subscription CRUD
async def subscribe_to_discussion(db: AsyncSession, user_id: str, discussion_id: int) -> DiscussionSubscription:
...
async def unsubscribe_from_discussion(db: AsyncSession, user_id: str, discussion_id: int) -> bool:
...
# Notification CRUD
async def create_reply_notifications(db: AsyncSession, discussion_id: int, reply_id: int, author_id: str):
    ...
    author_name = f"{author.first_name} {author.last_name}" if author.first_name else "Someone"
...
async def mark_notification_read(db: AsyncSession, notification_id: int, user_id: str) -> Optional[Notification]:
